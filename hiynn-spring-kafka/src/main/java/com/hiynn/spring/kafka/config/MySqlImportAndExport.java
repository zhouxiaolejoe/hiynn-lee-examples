...
public class MySqlImportAndExport {
    ...
    public static void main(String args[]) throws IOException {
        File f = new File("E:\\jdbc.properties");
        InputStream in = new FileInputStream(f);
        //InputStream is = MySqlImportAndExport.class.getClassLoader().getResourceAsStream("E:\\jdbc.properties");
...
//        String MysqlPath = properties.getProperty("jdbc.MysqlPath"); //  路径是mysql中 bin 文件 的位置

        /*StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("mysqldump").append(" --opt").append(" -h").append(host);
        stringBuilder.append(" --user=").append(username).append(" --password=").append(password).append(" --lock-all-tables=true");
        stringBuilder.append(" --result-file=").append(exportPath).append(" --default-character-set=utf8 ").append(exportDatabaseName + " ");*/


        //注意哪些地方要空格，哪些不要空格
        command.append("cmd /k mysqldump -u").append(username).append(" -p").append(password)//密码是用的小p，而端口是用的大P。
               .append(" -h").append(host).append(" -P").append(port).append(" ").append(exportDatabaseName).append(" -r ").append(exportPath);
        //String command = new String("cmd /k mysqldump -u"+username+" -p"+password+" "+exportDatabaseName+" >"+exportPath);
